...
export async function lambdaHandler(event) {
    ...
    try {
        ...
        if (body.simulateSlotFullError) {
            const currentSlot = await dynamoClient.send(new GetCommand({ TableName: slotsTable, Key: { id: slotId } }));
            if (!currentSlot.Item) {
        ...
        let updatedSlot = (await dynamoClient.send(new UpdateCommand({
        ...
        if (Number(updatedSlot.availableSeats ?? 0) <= 0 && updatedSlot.status !== 'full') {
            updatedSlot = (await dynamoClient.send(new UpdateCommand({
                TableName: slotsTable,
                Key: { id: slotId },
        ...
        if (error.name === 'ConditionalCheckFailedException') {
            const currentSlot = await dynamoClient.send(new GetCommand({ TableName: process.env.SLOTS_TABLE ?? '', Key: { id: event.pathParameters?.slotId } }));
        ...
        return errorResponse(500, 'Failed to book slot', { message: error.message });
    }
}
