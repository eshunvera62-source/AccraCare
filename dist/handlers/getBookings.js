...
export async function lambdaHandler(event) {
    ...
    try {
        ...
        if (queryParams.slotId && queryParams.slotId !== 'all') {
            ...
            do {
                const response = await dynamoClient.send(new ScanCommand({ TableName: bookingsTable, ExclusiveStartKey: exclusiveStartKey }));
        ...
        return errorResponse(500, 'Failed to fetch bookings', { message: error.message });
    }
}
